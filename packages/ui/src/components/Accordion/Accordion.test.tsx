		...
		render(
			<Accordion type="single" collapsible className="w-[600px]">
				<AccordionItem value="item-1">
					<AccordionTrigger>Are you a real person?</AccordionTrigger>
					<AccordionContent>
						No, I am an autonomous robot called <b>Hailee</b>, programmed by my
						father Eric Johnson, the Senior Engineer of WER Technologies, to
						help people with their troubles.
					</AccordionContent>
				</AccordionItem>
			</Accordion>
		);
		...
		render(
			<Accordion type="multiple" className="w-[600px]">
				...
				<AccordionItem value="item-2">
					<AccordionTrigger>What is hidden there?</AccordionTrigger>
					<AccordionContent>A simple text</AccordionContent>
				</AccordionItem>
			</Accordion>
		);
		const trigger1 = screen.getByRole("button", {
			name: /are you a real person/i,
		});
		const trigger2 = screen.getByRole("button", {
			name: /what is hidden there/i,
		});
